...
- [x] Cara azul — doce detecciones: [[Solicitud de TGT sin preautenticación]], [[Tickets de servicio con cifrado débil en volumen]], [[Replicación de directorio desde un origen no autorizado]], [[Autenticación NTLM donde el dominio usa Kerberos]], [[Ticket de servicio sin ticket inicial previo]], [[Acceso a LSASS desde proceso no firmado]], [[Escritura del atributo de delegación RBCD]], [[Conexión a host de resolución de nombres no autorizado]], [[Autenticación por certificado a cuenta privilegiada]], [[Impersonación por delegación S4U]], [[Certificado emitido con sujeto ajeno al solicitante]] y [[Cuenta de alto valor autenticándose a host con delegación]]
...
- [ ] **Detección de abuso de confianza — el único hueco azul que queda, y necesita técnica nueva, no una regla.** La escalada por SID History del vault ([[Escalada intra-bosque por SID History]]) inyecta el SID en el **PAC de un ticket forjado**, no en el atributo, así que no genera el evento de "SID History agregado" (`4765`) y solo la cubre parcialmente la detección de golden. Detectarla de verdad exige o bien inspeccionar el PAC del `4769` (avanzado, sin artefacto), o bien modelar la **variante de persistencia** —escribir el atributo `SIDHistory` con DSInternals/mimikatz, que sí emite `4765`—, que es una técnica roja que el vault todavía no tiene. Cerrar este hueco es agregar esa técnica de persistencia + el artefacto `4765` + su detección, no calibrar una regla
