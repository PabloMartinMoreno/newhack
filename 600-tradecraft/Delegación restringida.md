...
Esa firma es de buena fidelidad si se instrumenta el campo de servicios de tránsito, pero ninguna detección del vault la implementa —[[Tickets de servicio con cifrado débil en volumen]] consume el mismo `4769` pero busca cifrado débil, no la marca de S4U—. Queda como hueco de detección declarado en [[MOC - Active Directory]]: la telemetría (`4769`) está y se consume, pero falta la regla que ancle en el `S4U2Proxy` con impersonación de cuenta privilegiada.
