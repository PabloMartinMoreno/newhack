...
La cara azul del abuso de la CA es la misma asimetría que ADCS entero: la emisión y el uso del cert se ven mal (falta un artefacto de la CA y una detección de auth por certificado), pero las ramas de **relay** aterrizan en detecciones existentes y las de **modificación de configuración** dejan la firma de escritura de `4662`. La detección de autenticación por certificado queda como hueco declarado en [[MOC - Active Directory]].
