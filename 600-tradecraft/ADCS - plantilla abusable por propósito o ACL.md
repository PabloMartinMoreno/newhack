...
Ninguna detección del vault ancla específicamente en la autenticación por certificado —es un hueco declarado en [[MOC - Active Directory]]—: el `4768` con certificado se consume por las reglas de TGT existentes pero no hay una que distinga el cert-as-anyone. La señal escribible es la modificación de plantilla de ESC4/ESC13 sobre `4662`, análoga a RBCD. Como en toda la persistencia por certificado, el problema defensivo es que el cert **sobrevive al cambio de contraseña**: detectar el uso es tardío, la mitigación es de configuración de la PKI.
