...
Ninguna detección del vault ancla en el SID inyectado específicamente —es un hueco declarado en [[MOC - Active Directory]]—: la de golden ticket lo ve como un golden cualquiera, sin distinguir el SID de escalada. La señal de configuración de mayor retorno es auditar `SIDHistory`: una cuenta con el SID de un grupo privilegiado de otro dominio, sin migración de por medio, es la firma del ataque en reposo.
