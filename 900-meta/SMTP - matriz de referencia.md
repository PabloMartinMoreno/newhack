...
> **Puertos: 25** (SMTP servidor-a-servidor y legacy), **587** (submission, con auth y STARTTLS) y **465** (SMTPS, TLS implícito). Recon y abuso del servicio de correo. La inyección de cabeceras en una app web es otra cosa: [[MOC - Email header injection]]. Para **leer** buzones (la contracara), [[IMAP y POP3 - matriz de referencia]]. Criterio de recon en [[MOC - Reconocimiento de red]].
...
| `nmap -sV --script 'smtp*' -p25,465,587 HOST` | Comandos soportados, relay abierto, usuarios y, en Exchange, info NTLM |
...
| Comando | Qué hace |
|---|---|
| `HELO host` | Saludo; lista extensiones (`AUTH`, `STARTTLS`, `SIZE`). `HELO` es la versión vieja |
...
Wordlists de usuarios recomendadas:
